...
# 12
The project didn't turn out as we expected at milestone0, since we all only communicated thru text messages and never worked synchonrously the whole time. We faced a lot of difficulties on communication and designs since we all had different styles of implementation. It would've been better if we hosted a meeting every week to catch up and look at our progress and improve upon that.
